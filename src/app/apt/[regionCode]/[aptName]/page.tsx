...
export default function AptDetailPage() {
  ...
  if (loading) {
    return (
      <div className="text-center py-16">
        ...
        <p className="text-teal-600/60 mt-3">데이터 불러오는 중...</p>
      </div>
    );
  }

  if (!data) {
    return <div className="text-center py-16 text-teal-500/60">데이터를 불러올 수 없습니다</div>;
  }
  ...
  return (
    <div className="space-y-6">
      {/* 브레드크럼 */}
      <nav className="flex items-center gap-1.5 text-sm text-teal-600/60">
        <Link href="/" className="hover:text-teal-800 cursor-pointer transition-colors duration-200">홈</Link>
        <IconChevronRight className="w-3.5 h-3.5" />
        <Link href={`/search?code=${regionCode}`} className="hover:text-teal-800 cursor-pointer transition-colors duration-200">{data.region}</Link>
        <IconChevronRight className="w-3.5 h-3.5" />
        <span className="text-teal-900 font-medium">{aptName}</span>
      </nav>
      ...
      <div className="card">
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-2xl font-bold text-teal-900">{aptName}</h1>
            <p className="text-sm text-teal-600/60 mt-1">
        ...
        <div className="grid grid-cols-3 gap-3 mt-4">
          <div className="bg-teal-50/60 rounded-lg p-3 text-center">
            <div className="text-xs text-teal-600">평균</div>
            <div className="font-bold text-teal-900 num mt-1">{formatPrice(data.avgPrice)}</div>
          </div>
          <div className="bg-sky-50 rounded-lg p-3 text-center">
            <div className="text-xs text-sky-700">최저</div>
            <div className="font-bold text-sky-800 num mt-1">{formatPrice(data.minPrice)}</div>
          </div>
          <div className="bg-rose-50 rounded-lg p-3 text-center">
            <div className="text-xs text-rose-600">최고</div>
            <div className="font-bold text-rose-700 num mt-1">{formatPrice(data.maxPrice)}</div>
          </div>
        </div>

        {areas.length > 1 && (
          <div className="mt-4">
            <h4 className="text-sm font-medium text-teal-700 mb-2">면적별 평균</h4>
            <div className="flex flex-wrap gap-2">
              {areas.map(a => (
                <div key={a.pyeong} className="bg-teal-50/50 rounded-lg px-3 py-2 text-sm">
                  <span className="font-medium text-teal-800">{a.pyeong}평</span>
                  <span className="text-teal-600/70 ml-2 num">{formatPrice(a.avgPrice)}</span>
                  <span className="text-teal-500/40 ml-1 text-xs">({a.count}건)</span>
                </div>
              ))}
            </div>
          </div>
