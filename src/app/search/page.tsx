  ...
  if (!code) {
    return (
      <div className="space-y-6">
        <SearchBar size="lg" />
        <div className="text-center text-gray-400 py-16">지역을 검색해주세요</div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      {loading && (
        <div className="text-center py-16">
          ...
          <p className="text-gray-500 mt-3">실거래 데이터 불러오는 중...</p>
        </div>
      )}
      {data && !loading && (
        <>
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">{data.region}</h1>
          ...
          {data.complexes.length > 0 && (
            <div className="card">
              <h3 className="font-semibold text-gray-900 mb-3">단지별 거래 현황</h3>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-1.5">
                {data.complexes.map(c => (
                  <a key={c.name} href={`/apt/${data.regionCode}/${encodeURIComponent(c.name)}`} className="flex items-center gap-2.5 p-2.5 rounded-lg hover:bg-indigo-50 text-sm cursor-pointer transition-colors duration-150">
...
export default function SearchPage() {
  return (
    <Suspense fallback={<div className="text-center py-16 text-gray-400">로딩중...</div>}>
      <SearchContent />
    </Suspense>
  );
}
